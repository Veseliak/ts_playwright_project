...
export class PageObject extends AbstractPage {
  ...
  readonly displayName = '#displayFirstName';
  ...
  async fillFirstName(firstName: string): Promise<void> {
    await this.input.fillInput(this.firstNameInputSelector, firstName);
  }

  async fillAge(age: string): Promise<void> {
    await this.input.fillInput(this.ageInputSelector, age);
  }
